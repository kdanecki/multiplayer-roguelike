...
func _physics_process(_delta):
	...
	var query = PhysicsRayQueryParameters2D.create(position, position + attack_direction*100)
...
func attack():
	if ($AnimatedSprite2D.get_animation() == "attack" and $AnimatedSprite2D.frame == 2) or ($AnimatedSprite2D.get_animation() == "attack2" and $AnimatedSprite2D.frame == 3):
		var hit = target as PlayerBase
		if hit:
			hit.damage(5)
			print("slash", target)
		else:
